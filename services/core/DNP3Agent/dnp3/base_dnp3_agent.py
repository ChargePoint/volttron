...
class BaseDNP3Agent(Agent):
    ...
    def onstart(self, sender, **kwargs):
        ...
        _log.debug('Starting DNP3Outstation')
    ...
    def get_selector_block(self, point_name, edit_selector):
        ...
        """
        _log.debug('Get point values for selector block {}, index {}'.format(point_name, edit_selector))
        point_values = {}
        try:
    ...
    def set_points(self, point_dict):
        ...
        """
        _log.debug('Setting DNP3 point values: {}'.format(point_dict))
        try:
    ...
    def get_point_definitions(self, point_name_list):
        ...
        """
        _log.debug('Fetch a list of DNP3 point definitions for {}'.format(point_name_list))
        try:
            response = {}
            for name in point_name_list:
                point_def = self.point_definitions.get_point_named(name)
