...
class DNP3Outstation(opendnp3.IOutstationApplication):
    ...
    def __init__(self, local_ip, port, outstation_config):
        ...
        self.stack_config = asiodnp3.OutstationStackConfig(opendnp3.DatabaseSizes.AllTypes(outstation_config.get('database_sizes', 10)))
        self.stack_config.outstation.eventBufferConfig = opendnp3.EventBufferConfig.AllTypes(outstation_config.get('event_buffers', 10))
        ...
        for point in self.get_agent().point_definitions.all_points():
            # if point.point_type in ['Analog Input', 'Analog Output']:
            #     cfg = db_config.analog[int(point.index)]
            # elif point.point_type in ['Binary Input', 'Binary Output']:
    ...
    def set_agent(cls, agent):
        ...
        # @todo Handle scenarios in which this doesn't get set (invocation from the command line, for instance).
        cls.agent = agent
    ...
    def dnp3_log_level(self, outstation_config):
        ...
        """
        log_level_str = outstation_config.get('log_levels', 'NORMAL').upper()
        ...
        log_level = 0
        for name in log_level_str.replace(" ", "").split("|"):
            log_level = log_level | name_to_bitmasks.get(name, 0)

        _log.debug('Setting DNP3 log level={} ({})'.format(log_level, log_level_str))
        return log_level
